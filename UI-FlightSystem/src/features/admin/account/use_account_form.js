...
import { CREATE_DEFAULT_VALUES, EDIT_DEFAULT_VALUES, BE_FIELD_MAP, COMMON_RULES, CREATE_ONLY_RULES } from "./account_constants";

function buildPayload(values, isEdit, userId) {
    const { confirmPassword, birthday, roleNames, userName, ...rest } = values;
    const payload = { ...rest, birthday: birthday?.trim() || null, roleNames: [roleNames] };
    if (isEdit) {
