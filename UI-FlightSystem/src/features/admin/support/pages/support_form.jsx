...
import { REQUEST_TYPE_LABEL } from "../support_constants";
import { formatDate } from "@/utils/date_utils";
import "./support.css";

function SupportForm({requestId, onClose, onSuccess}) {
    const dispatch = useDispatch();
    const { selectedItem: detail, isDetailLoading, actionLoading, actionError,
    } = useSelector((state) => state.supportRequest);

    useEffect(() => {
        dispatch(fetchSupportRequestById(requestId));
    }, [dispatch, requestId]);
    ...
    const handleReject = async () => {
        const result = await dispatch(
            rejectSupportRequest(requestId)
        );
        ...
        <div className="modal_overlay">
            <div className="form_modal">
                ...
                <div className="form_modal_content">
                    ...
                    {!isDetailLoading && detail && (
                        <>
                            ...
                            <div className="form_group" style={{ marginBottom: 16 }}>
                                <label>Lý do từ khách hàng</label>
                                <textarea value={detail.reason ?? ""} disabled style={{ minHeight: 90 }}/>
                            </div>

                            {detail.adminNote && (
                                <div className="form_group">
                                    <label>Ghi chú của Admin</label>
                                    <textarea value={detail.adminNote} disabled style={{ minHeight: 90 }}/>
                                </div>
                            )}
                            ...
                            {detail.newFlight && (
                                <div className="modal_section_block">
                                    <h4 className="section_subtitle">
                                        ...
                                        {" "}
                                        Thông tin chuyến bay mới đề xuất
                                    </h4>
                                    <div className="form_group" style={{ marginBottom: 10 }}>
                                    ...
                                    <div className="form_grid_two">
                                        <div className="form_group">
                                            ...
                                            <input type="text" value={new Date(
                                                    detail.newFlight.departureTime
                ...
                {isPending && !isDetailLoading && (
                    <div className="form_modal_footer" style={{padding: 20}}>
                        <button type="button" className="btn_cancel btn_reject_custom" onClick={handleReject} disabled={actionLoading}>
                            {actionLoading ? "Đang xử lý..." : "Từ chối"}
                        </button>
                        <button type="button" className="btn_submit" onClick={handleApprove} disabled={actionLoading}>
