...
import { CREATE_DEFAULT_VALUES, EDIT_DEFAULT_VALUES, BE_FIELD_MAP, COMMON_RULES } from "./plane_constants";
...
export function usePlaneForm({mode, planeData, onSuccess, onClose}) {
    ...
    const { register: baseRegister, control, formState, onSubmit, setValue,
    } = useCrudForm({
        isEdit,
        ...
        onSubmitApi: (values) => {
            ...
            return isEdit ? planeService.update(planeData.planeId, payload) : planeService.create(payload);
        },
        onSuccess,
        onClose,
    });
