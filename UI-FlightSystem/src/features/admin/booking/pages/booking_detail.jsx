...
import { formatDateShort, formatTime } from "@/utils/date_utils";
import { BOOKING_STATUS_LABEL, PASSENGER_TABLE_HEADS, PASSENGER_TYPE_LABEL, SEAT_CLASS_OPTIONS, TRIP_TYPE_LABEL } from "../booking_constants";
import { fetchBookingById, clearDetail } from "../detail_slice";
import "./detail.css";
...
function BookingDetail() {
    ...
    const { detail, isLoadingDetail, error } = useSelector((state) => state.bookingDetail);
    ...
    if (error) {
        return (
            <div className="state_message error_alert">
    ...
    if (!detail) {
        return (
            <div className="state_message">
                Không tìm thấy dữ liệu đơn đặt vé.
            </div>
        );
    }
    ...
    return (
        <div className="detail_container">
            ...
            <div className="flight_card">
                {detail.flights?.map((flight, index) => (
                    <div key={flight.flightId || index} className="flight_leg">
                        ...
                        <div className="segment_timeline">
                            {(flight.segments?.length ? flight.segments : [flight]
                                ...
                                <div key={segment.segmentOrder || segmentIndex} className="segment_node">
                                    <div className="segment_detail">
                                        <div className="segment_time">
                                            {formatTime(segment.departureTime)}{" — "}{formatTime(segment.arrivalTime)}
                                            <span>{formatDateShort(segment.departureTime)}</span>
                                        </div>
                                        <div className="segment_airport">
                                            <strong>{segment.originAirport}</strong>
                                            ({segment.originAirportName})
                                            <i className="bx bx-right-arrow-alt" />
                                            <strong>{segment.destinationAirport}</strong>
