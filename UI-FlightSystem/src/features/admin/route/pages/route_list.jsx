...
import { TABLE_HEADS, ROUTE_STATUS_LABEL } from "../route_constants";
...
function RouteList() {
    ...
    const { items, pageIndex, pageSize, totalPages, totalCount, error, isLoading, isDetailLoading, refresh,
        status, originAirportCode, destinationAirportCode
    } = useDataList({
        sliceName: "route",
        fetchListThunk: fetchRoutes,
    ...
    const { loadDetail } = useDetailLoader({
        fetchByIdThunk: fetchRouteById,
        onFulfilled: (data) => openEdit(data),
    });

    const toggleDelete = useDeleteConfirm({
        onSuccess: refresh,
        deleteServiceFn: routeService.delete,
    });
    ...
    const airportOptions = useMemo(
        () => airports.map((a) => ({ id: a.airportCode, name: `${a.airportCode} — ${a.city}` })),
        [airports]
    );
    ...
    return (
        <div className="list_container">
            <div className="list_header">
                ...
                <button onClick={openAdd} className="btn_add"><i className="bx bx-plus" /> Thêm tuyến bay</button>
            </div>
                    ...
                    render={(route, index) => (
                        <tr key={route.routeId}>
                            ...
                            <td className="action_buttons_group">
                                <button onClick={() => loadDetail(route.routeId)} title="Sửa" className="btn_action" disabled={isDetailLoading}>
            ...
            <RouteForm
                key={formState.data?.routeId ?? "new"}
                isOpen={formState.isOpen}
                onClose={closeModal}
                onSave={refresh}
                routeData={formState.data}
                mode={formState.mode}
            />
                ...
                <div className="modal_overlay animate_fade_in">
                    <div className="modal_box animate_slide_up">
                        ...
                        <div className="modal_footer">
                            ...
                            <button className="btn_submit btn_danger_action" disabled={toggleDelete.isLoading} onClick={() => toggleDelete.confirm(item => item.routeId)}>
