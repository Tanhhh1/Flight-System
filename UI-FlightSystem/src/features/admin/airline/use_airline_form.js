...
import { CREATE_DEFAULT_VALUES, EDIT_DEFAULT_VALUES, BE_FIELD_MAP, COMMON_RULES } from "./airline_constants";
...
export function useAirlineForm({ mode, airlineData, onSuccess, onClose }) {
    ...
    const { register: baseRegister, control, setValue, formState, onSubmit,
    } = useCrudForm({
        isEdit,
        ...
        onSubmitApi: (values) => {
            ...
            return isEdit ? airlineService.update(airlineData.airlineId, payload) : airlineService.create(payload);
        },
        onSuccess,
        onClose,
    });
