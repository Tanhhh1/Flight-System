...
import { currentDate, formatTime, formatDateShort } from "@/utils/date_utils";
...
import { TABLE_HEADS, FLIGHT_STATUS_LABEL } from "../flight_constants";
...
function FlightList() {
    ...
    const { items, pageIndex, pageSize, totalPages, totalCount, error, isLoading, refresh,
        originAirportCode, destinationAirportCode, departureDate, statusFilter, airlineFilter
    } = useDataList({
        sliceName: "flight",
        fetchListThunk: fetchFlights,
        selectExtraFilters: (state) => ({
            originAirportCode: state?.originAirportCode,
            destinationAirportCode: state?.destinationAirportCode,
            departureDate: state?.departureDate,
            status: state?.statusFilter,
            airlineId: state?.airlineFilter,
        }),
    });

    const toggleActive = useDeleteConfirm({
        onSuccess: refresh,
        deleteServiceFn: (id) => flightService.delete(id),
    });
    ...
    const createFlightPath = `${PATHS.ADMIN.ROOT}/${PATHS.ADMIN.FLIGHT_NEW}`;
    ...
    return (
        <div className="list_container">
            <div className="list_header">
                ...
                <button onClick={() => navigate(createFlightPath)} className="btn_add"><i className="bx bx-plus" /> Thêm chuyến bay</button>
            </div>

            <div className="list_toolbar">
                <div className="filter_select">
                    <SearchableSelect data={airportOptions} value={originAirportCode} onChange={(val) => dispatch(setOriginAirportCode(val))} placeholder="Tất cả điểm đi" itemKey="id" displayValue="name" searchFields={["name"]} />
                </div>
                <div className="filter_select">
                    <SearchableSelect data={airportOptions} value={destinationAirportCode} onChange={(val) => dispatch(setDestinationAirportCode(val))} placeholder="Tất cả điểm đến" itemKey="id" displayValue="name" searchFields={["name"]} />
                </div>
                <div className="filter_select">
                    <input type="date" value={departureDate} onChange={(e) => dispatch(setDepartureDate(e.target.value))} className="date_input" />
                </div>
                <div className="filter_select">
                    <SearchableSelect data={airlineOptions} value={airlineFilter} onChange={(val) => dispatch(setAirlineFilter(val))} placeholder="Tất cả hãng bay" itemKey="id" displayValue="name" searchFields={["name"]} />
                </div>
                    ...
                    render={(flight, index) => (
                        <tr key={flight.flightId}>
                            ...
                            <td className="action_buttons_group">
                                <button onClick={() => navigate(`${PATHS.ADMIN.ROOT}/${PATHS.ADMIN.FLIGHT_EDIT.replace(":id", flight.flightId)}`)} title="Sửa" className="btn_action">
                ...
                <div className="modal_overlay animate_fade_in">
                    <div className="modal_box animate_slide_up">
                        ...
                        <div className="modal_footer">
                            ...
                            <button className="btn_submit btn_danger_action" disabled={toggleActive.isLoading} onClick={() => toggleActive.confirm((item) => item.flightId)}>
