...
import { CREATE_DEFAULT_VALUES, EDIT_DEFAULT_VALUES, BE_FIELD_MAP, COMMON_RULES } from "./airport_constants";
...
export function useAirportForm({ mode, airportData, onSuccess, onClose, }) {
    ...
    const { register: baseRegister, control, setValue, formState, onSubmit } = useCrudForm({
        ...
        onSubmitApi: (values) => {
            ...
            return isEdit ? airportService.update(airportData.airportId, payload) : airportService.create(payload);
        },
        onSuccess,
        onClose,
    });
