...
import { PAYMENT_METHOD_OPTIONS, TRIP_TYPE_LABEL } from "../payment_constants";
import AlertModal from "@/components/common/alert_modal";
import { formatTime, formatDateShort } from "@/utils/date_utils";
import "./payment.css";

function PaymentMethod() {
    const { booking, selectedMethod, setSelectedMethod,
        loading, fetchingBooking, error, clearError, isFailed,
        isPending, handleConfirmPayment,
    } = usePayment();
                    ...
                    <div className="payment_main_content">
                        ...
                        <div className="payment_methods_panel">
                            <div className="panel_header">
                                <h4>
                                    <i className="bx bx-shield-quarter" /> Chọn phương thức thanh toán
                                </h4>
                            </div>
                        ...
                        <div className="form_submit_row">
                            <button type="button" className="btn_confirm_payment" onClick={handleConfirmPayment} disabled={loading || (!isPending && !isFailed)}>
                                {loading ? (
                            ...
                            <div className="panel_body summary_body">
                                <div className="summary_booking_info">
                                    ...
                                    <div className="summary_meta_row">
                                        ...
                                        <span className="summary_flight_value">
                                            {booking.className}
                                        </span>
                                    </div>
                                    <div className="summary_meta_row">
                                ...
                                {booking.flights.map((flight, idx) => (
                                    <div key={flight.flightId} className="summary_flight_item">
                                        ...
                                        <p className="summary_flight_route">{flight.originAirport} → {flight.destinationAirport} </p>
                                        <span className="summary_flight_detail">
                                            {formatTime(flight.departureTime)} ·{" "}
                                            {formatDateShort(flight.departureTime)}
                                        </span>
                                        <span className="summary_flight_detail">{flight.airlineName} · {flight.planeModel}</span>
                                    </div>
                                ))}
                                <div className="payment_summary_divider" />
                                ...
                                <p className="secure_payment_note">
                                    <i className="bx bx-check-shield" /> Dữ liệu thanh toán được mã hóa
                                    SSL 256-bit an toàn tuyệt đối.
                                </p>
                            </div>
                        </div>
                    </aside>
