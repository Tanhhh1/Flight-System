...
import { seatApi } from "./seat_service";
import { parseApiError } from "@/lib/redux/parse_api_error";
import { SEAT_REVERSE_STORAGE_KEY, SEAT_STATUS } from "@/features/client/seat/seat_constants";
...
export const verifyBookingCode = createAsyncThunk(
    ...
    async (bookingCode, { rejectWithValue }) => {
        try {
            const { data } = await seatApi.verifyBookingCode(bookingCode);
            if (!data.succeeded)
...
export const restoreBookingFromSession = createAsyncThunk(
    ...
    async (_, { rejectWithValue }) => {
        ...
        try {
            const { data } = await seatApi.verifyBookingCode(session.bookingCode);
            if (!data.succeeded) {
                clearSession();
...
export const fetchSeatMap = createAsyncThunk(
    ...
    async ({ flightId, bookingId }, { rejectWithValue }) => {
        try {
            const { data } = await seatApi.getSeatMap(flightId, bookingId);
            if (!data.succeeded)
...
export const holdSeat = createAsyncThunk(
    ...
    async ({ bookingId, flightId, seatId, passengerId }, { rejectWithValue }) => {
        try {
            const { data } = await seatApi.holdSeat({ bookingId, flightId, seatId, passengerId });
            if (!data.succeeded)
...
export const releaseSeat = createAsyncThunk(
    ...
    async ({ bookingId, flightId, flightSeatId, seatId, passengerId }, { rejectWithValue }) => {
        try {
            const { data } = await seatApi.releaseSeat({ bookingId, flightId, flightSeatId, passengerId });
            if (!data.succeeded)
...
export const confirmSeats = createAsyncThunk(
    ...
    async ({ bookingId, assignments }, { rejectWithValue }) => {
        try {
            ...
            const { data } = await seatApi.confirmSeats({
                bookingId,
            ...
            .addCase(fetchSeatMap.fulfilled, (state, action) => {
                ...
                state.pendingAssignments[flightId].forEach(assignment => {
                    if ((!assignment.seatId || assignment.seatId === 0) && assignment.flightSeatId > 0) {
                        for (const group of seatMap.classGroups) {
                            for (const row of group.rows) {
                                const seat = row.seats.find(s => s.flightSeatId === assignment.flightSeatId);
                                if (seat) {
...
export const {
    ...
    clearActionError, clearVerifyError, updateSeatStatusFromHub,
    resetSeatReverse, resetConfirmed
