...
import { useSeatSelection } from "../hooks/use_seat_selection";
import { useSeatHub } from "../hooks/use_seat_hub";
import PassengerPanel from "../pages/components/passenger_panel";
import SeatMap from "../pages/components/seat_map";
import AlertModal from "@/components/common/alert_modal";
import { PATHS } from "@/app/routes/paths";
import { resetConfirmed } from "../seat_slice";
import { SEAT_CLASSES_NAMES } from "@/features/admin/booking/booking_constants";
import "./styles/seat_selection.css";
...
function SeatSelection() {
    ...
    useEffect(() => {
        if (!bookingInfo) {
            handleRestore().catch(() => navigate(`${PATHS.CLIENT.PROFILE.ROOT}/${PATHS.CLIENT.PROFILE.VERIFY}`));
        }
    }, []);
    ...
    const handleAlertClose = useCallback(() => {
        ...
        if (wasSuccess) {
            handleRestore().then(() => {
                if (activeFlightId) handleFetchSeatMap(activeFlightId);
            });
        }
    ...
    const handleSeatClickWithConfirm = useCallback((seat) => {
        ...
        setDialog({
            message: `Giữ ghế ${seat.seatNumber} cho ${passengerName}?`,
            onConfirm: () => { setDialog(null); handleSeatClick(seat); },
        });
    ...
    const handleConfirmWithDialog = useCallback(() => {
        ...
        setDialog({
            message: `Xác nhận đặt ${totalPending} ghế cho đơn đặt vé này?`,
            onConfirm: () => { setDialog(null); handleConfirm(); },
        });
        ...
        <div className="seat_selection">
            ...
            {flights.length > 1 && (
                <div className="seat_selection_flight_tabs">
                    {flights.map((f) => (
                        <button
                            key={f.flightId}
                            className={`flight_tab ${activeFlightId === f.flightId ? "flight_tab_active" : ""}`}
                            onClick={() => handleSelectFlight(f.flightId)}
                        >
            ...
            <div className="seat_selection_footer">
                ...
                <div className="seat_selection_actions">
                    <button
                        className="seat_selection_btn_cancel"
                        onClick={() => {
                            handleReset();
                            navigate(`${PATHS.CLIENT.PROFILE.ROOT}/${PATHS.CLIENT.PROFILE.VERIFY}`);
                        }}
                    ...
                    <button
                        className="seat_selection_btn_confirm"
                        onClick={handleConfirmWithDialog}
                        disabled={isConfirming || totalPendingToConfirm === 0}
                    >
