...
import { useBookingForm } from "../use_booking_form";
import "./booking.css";

export default function BookingPage() {
    const { passengers, passengerTypes, pricePerPassenger, grandTotal, loading,
        clearError, error, selectedLegs, handleChange, addPassenger, removePassenger, handleSubmit,
    } = useBookingForm();

    const countryOptions = countries
        .map((c) => ({ code: c.cca2, name: c.translations?.vie?.common ?? c.name.common }))
    ...
    return (
        <>
            {error && <AlertModal type="error" message={error} onClose={clearError} />}
                            ...
                            <div className="summary_body">
                                ...
                                <div className="summary_price_breakdown">
                                    ...
                                    {passengerTypes.map((type) => {
                                        const count = passengers.filter((p) => Number(p.typeId) === type.typeId).length;
                                        ...
                                        const subtotal = indices.reduce(
                                            (sum, i) => sum + (pricePerPassenger[i] ?? 0),
                                            0
                                        );
