...
import { TRIP_TYPE_MAP, DEFAULT_PASSENGER } from "@/features/admin/booking/booking_constants";
...
export function useBookingForm() {
    ...
    const addPassenger = useCallback(() => {
        setPassengers((prev) => [ ...prev, { ...DEFAULT_PASSENGER, typeId: passengerTypes[0]?.typeId ?? null }]);
    }, [passengerTypes]);
    ...
    const handleSubmit = async () => {
        const isMissingInfo = passengers.some((p) => !p.fullName.trim() || !p.dob || !p.typeId);
        if (isMissingInfo) {
        ...
        try {
            ...
            navigate(`${PATHS.CLIENT.PAYMENT}?bookingId=${bookingId}&${searchParams.toString()}`);
        } catch (err) {
            const message =
    ...
    return { passengers, passengerTypes, pricePerPassenger, grandTotal, loading, clearError,
